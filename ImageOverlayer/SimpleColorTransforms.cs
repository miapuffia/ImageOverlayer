...
namespace Sc.Util.Rendering {
	...
	public static class SimpleColorTransforms {
		...
		public static Color TransformBrightness(
			...
			(double H, double S, double L) hsl = colorTransformMode == ColorTransformMode.Hsl
					? SimpleColorTransforms.RgBtoHsl(color)
					: SimpleColorTransforms.RgBtoHsb(color);
			if((SystemMath.Abs(hsl.L) < SimpleColorTransforms.tolerance)
					&& (brightnessTransform > 1D))
				hsl.L = brightnessTransform - 1D;
			else
				hsl.L *= brightnessTransform;
			return colorTransformMode == ColorTransformMode.Hsl
					? SimpleColorTransforms.HsLtoRgb(hsl.H, hsl.S, hsl.L, outputAlpha ?? color.A)
					: SimpleColorTransforms.HsBtoRgb(hsl.H, hsl.S, hsl.L, outputAlpha ?? color.A);
		}
		...
		public static Color TransformSaturationAndBrightness(
			...
			(double H, double S, double L) hsl = colorTransformMode == ColorTransformMode.Hsl
					? SimpleColorTransforms.RgBtoHsl(color)
					: SimpleColorTransforms.RgBtoHsb(color);
			if((SystemMath.Abs(hsl.S) < SimpleColorTransforms.tolerance)
					&& (saturationTransform > 1D))
				hsl.S = saturationTransform - 1D;
			else
				hsl.S *= saturationTransform;
			if((SystemMath.Abs(hsl.L) < SimpleColorTransforms.tolerance)
					&& (brightnessTransform > 1D))
				hsl.L = brightnessTransform - 1D;
			else
